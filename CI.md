...
- `docs:` is a non-releasing Conventional Commit type, so a docs-only fix
  does not open a Release PR by itself. An urgent user-facing docs
  correction should be typed `fix:` so release-please cuts a patch release.
