...
// Compiled once at package load: templatesEqual runs on every plan/apply via the
// jobTemplateType semantic-equality check, so these must not be recompiled per call.
var (
